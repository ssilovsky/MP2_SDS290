...
We can look then at a Bonferoni confidence intervals for a more conservative look at which pairwise comparisons are significant. We only find muBusy.Alone-muEmpty.Alone to be significant. Thus we see more support for the idea that when a person is alone in a dining hall, their response depends and differ significantly based on whether a dining hall is busy or empty.
