...
For our analysis, we will be focusing on the `sit_and_eat` response. THis is because our 3 other response groups, `find_new_dining` `grab_n_go` `enjoy_eating`, all showed signficantly more violations of pre-conditon checks such as the equal variance condition. 
...
Now that we've looked at our pre-conditions, we can fit our 2 way ANOVA. We can then see that we only have one significant coefficient with a p-value of 0.0306, `busy`. However, `company` and our interactive coefficient lack significant p-values.
...
We can then look at the interaction plot for the mean responses of `sit_and_eat` by `busy` and `company` treatment groups. The plot suggests the effect of the company an individual has, is dependent on the bussiness level, most specifically when its the highest level of bussiness. 

```{r echo=FALSE, message=FALSE, warning=FALSE}
with(df_dropped,interaction.plot(busy, company, sit_and_eat))
```

We can now look at the Tukey HSD to find signicant pair-wise comparisons of our variables' levels. For `busy`, we see a significant pairwise comparison for when we go from empty to busy dining halls meaning responses are significantly different when we switch between these levels, regardless of company. Our interaction variable also has a significant pairwise comparison for Empty:Alone-Busy:Alone meaning when a person is alone in a dining hall, their response depends and differ significnatly based on whether a dining hall is busy or empty.

```{r echo=FALSE, message=FALSE, warning=FALSE}
TukeyHSD(dining_aov)
```

We can look then at a Bonferoni confidence intervals for a tougher look at what pairwise comparisons are signficant. We only find muBusy.Alone-muEmpty.Alone to be signficant. Thus we see more support for the idea that when a person is alone in a dining hall, their response depends and differ significnatly based on whether a dining hall is busy or empty.
