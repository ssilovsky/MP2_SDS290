...
dining_aov <- aov(sit_and_eat~var1*var2, data=df_dropped)
...
```
```{r}
TukeyNonaddPlot(sit_and_eat~var1*var2, data=df_dropped)
...
```{r}
df$sd_se <- sd(df$sit_and_eat)
df$sd_ee <- sd(df$enjoy_eating)
df$mean_se <- mean(df$sit_and_eat)
df$mean_ee <- mean(df$enjoy_eating)

ggplot()
```

```{r}
cellmeans <- df_dropped |>
  group_by(var1, var2)|>
  summarise(mean=log(mean(sit_and_eat)), sd=sd(sit_and_eat))


ggplot(cellmeans, aes(x=mean, y=sd))+ geom_point()

summary(lm(sd~mean, data=cellmeans))
```
