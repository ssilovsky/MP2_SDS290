...
```{r echo=FALSE, message=FALSE, warning=FALSE}
data <- read_csv('C:/Users/sophi/Downloads/sds 290 - research design and analysis/Mini-project+2_April+25,+2023_08.56.csv')
```
...
df <- data |> 
  ...
  rename("consent"="Q1", 
         "dietary_resitriction"="Q2...19", 
         "class" ="Q2...20", "gender"="Q4", 
         "neighborhood"="Q5", 
         "sit_and_eat"="Q16_1", 
...
```{r}
test1_sit <- aov(sit_and_eat ~ dietary_resitriction + treatment, data = df)

summary(test1_sit)
```
```{r}
test2_sit <- lm(sit_and_eat ~ dietary_resitriction + treatment, data = df)

summary(test2_sit)
```

```{r}
test1_din <- aov(find_new_dining ~ dietary_resitriction + treatment, data = df)

summary(test1_din)
```

```{r}
test2_din <- lm(find_new_dining ~ dietary_resitriction + treatment, data = df)

summary(test2_din)
```

```{r}
test1_grab <- aov(grab_and_go ~ dietary_resitriction + treatment, data = df)

summary(test1_grab)
```

```{r}
test2_grab <- lm(grab_and_go ~ dietary_resitriction + treatment, data = df)

summary(test2_grab)
```

```{r}
test1_enj <- aov(enjoy_eating ~ dietary_resitriction + treatment, data = df)

summary(test1_enj)
```

```{r}
test2_enj <- lm(enjoy_eating ~ dietary_resitriction + treatment, data = df)
